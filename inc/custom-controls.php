...
if ( class_exists( 'WP_Customize_Control' ) ) {
	...
	class Skyrocket_Alpha_Color_Control extends Skyrocket_Custom_Control {
		...
		public function enqueue() {
			...
			wp_enqueue_script( 'skyrocket-custom-controls-js', $this->get_skyrocket_resource_url() . 'js/customizer.js', array( 'jquery', 'wp-color-picker-alpha' ), '1.1', true );
			wp_enqueue_style( 'skyrocket-custom-controls-css', $this->get_skyrocket_resource_url() . 'css/customizer.css', array(), '1.0', 'all' );
	...
	/**
	 * Simple Two Color Gradient Custom Control
	 *
	 * @author Anthony Hortin <http://maddisondesigns.com>
	 * @license http://www.gnu.org/licenses/gpl-2.0.html
	 * @link https://github.com/maddisondesigns
	 ...
	 * @link https://github.com/kallookoo/wp-color-picker-alpha
	 */
	class Skyrocket_Simple_Two_Color_Gradient_Control extends Skyrocket_Custom_Control {
		/**
		 * The type of control being rendered
		 */
		public $type = 'two-color-gradient';
		/**
		 * Start ColorPicker Attributes
		 */
		public $attributesStart = "";
		/**
		 * End ColorPicker Attributes
		 */
		public $attributesEnd = "";
		/**
		 * Color palette defaults
		 */
		public $defaultPalette = array(
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$colors = skyrocket_get_colors_from_gradient( $this->value() );
			$this->angle = $colors[0];
			$this->startColor = skyrocket_hex_rgba_sanitization( $colors[1], false);
			$this->endColor = skyrocket_hex_rgba_sanitization( $colors[count($colors)-1], false);

			$this->attributesStart .= 'data-default-color="' . esc_attr( $this->startColor ) . '"';
			$this->attributesStart .= 'data-alpha-enabled="true"';
			$this->attributesStart .= 'data-alpha-reset="' . ( isset( $this->input_attrs['resetalpha'] ) ? $this->input_attrs['resetalpha'] : 'true' ) . '"';
			$this->attributesStart .= 'data-alpha-custom-width="0"';

			$this->attributesEnd .= 'data-default-color="' . esc_attr( $this->endColor ) . '"';
			$this->attributesEnd .= 'data-alpha-enabled="true"';
			$this->attributesEnd .= 'data-alpha-reset="' . ( isset( $this->input_attrs['resetalpha'] ) ? $this->input_attrs['resetalpha'] : 'true' ) . '"';
			$this->attributesEnd .= 'data-alpha-custom-width="0"';
		}
		/**
		 * Enqueue our scripts and styles
		 */
		...
		public function to_json() {
			parent::to_json();
			$this->json['gradientcolorpickerpalette'] = isset( $this->input_attrs['palette'] ) ? $this->input_attrs['palette'] : $this->defaultPalette;
		}
		/**
		 * Render the control in the customizer
		 */
		public function render_content() {
		?>
		  <div class="two_color_gradient_control">
				<?php if( !empty( $this->label ) ) { ?>
					<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
				<?php } ?>
				<?php if( !empty( $this->description ) ) { ?>
					<span class="customize-control-description"><?php echo esc_html( $this->description ); ?></span>
				<?php } ?>
				<div class="two_color_gradient_control_color_sample" style="background-image:<?php echo esc_attr( $this->value() ); ?>"></div>
				<input type="hidden" id="<?php echo esc_attr( $this->id ); ?>" name="<?php echo esc_attr( $this->id ); ?>" value="<?php echo esc_attr( $this->value() ); ?>" class="customize-control-two-color-gradient" <?php $this->link(); ?> />
				<input type="hidden" class="two_color_gradient_control_colors" value="<?php echo esc_attr( $this->startColor ) . '|' . esc_attr( $this->endColor ); ?>"/>
				<input type="hidden" class="two_color_gradient_control_angle" value="<?php echo esc_attr( $this->angle );?>"/>
				<div class="two_color_gradient_control_arrow_selectors">
					<span class="dashicons dashicons-arrow-right-alt<?php echo ( $this->angle == 'to top' ? ' active' : '') ?>" data-iconangle="270" data-gradientangle="to top"></span>
					<span class="dashicons dashicons-arrow-right-alt<?php echo ( $this->angle == 'to right top' ? ' active' : '') ?>" data-iconangle="315" data-gradientangle="to right top"></span>
					<span class="dashicons dashicons-arrow-right-alt<?php echo ( $this->angle == 'to right' ? ' active' : '') ?>" data-iconangle="0" data-gradientangle="to right"></span>
					<span class="dashicons dashicons-arrow-right-alt<?php echo ( $this->angle == 'to right bottom' ? ' active' : '') ?>" data-iconangle="45" data-gradientangle="to right bottom"></span>
					<span class="dashicons dashicons-arrow-right-alt<?php echo ( $this->angle == 'to bottom' ? ' active' : '') ?>" data-iconangle="90" data-gradientangle="to bottom"></span>
					<span class="dashicons dashicons-arrow-right-alt<?php echo ( $this->angle == 'to left bottom' ? ' active' : '') ?>" data-iconangle="135" data-gradientangle="to left bottom"></span>
					<span class="dashicons dashicons-arrow-right-alt<?php echo ( $this->angle == 'to left' ? ' active' : '') ?>" data-iconangle="180" data-gradientangle="to left"></span>
					<span class="dashicons dashicons-arrow-right-alt<?php echo ( $this->angle == 'to left top' ? ' active' : '') ?>" data-iconangle="225" data-gradientangle="to left top"></span>
					<span class="dashicons dashicons-image-rotate<?php echo ( $this->angle == 'circle' ? ' active' : '') ?>" data-iconangle="105" data-gradientangle="circle"></span>
				</div>
				<input type="text" class="two_color_gradient_start_color_picker" id="<?php echo esc_attr( $this->id ); ?>_start" value="<?php echo esc_attr( $this->startColor ); ?>" class="customize-control-two-color-gradient-start" <?php echo $this->attributesStart; ?> />
				<input type="text" class="two_color_gradient_end_color_picker" id="<?php echo esc_attr( $this->id ); ?>_end" value="<?php echo esc_attr( $this->endColor ); ?>" class="customize-control-two-color-gradient-end" <?php echo $this->attributesEnd; ?> />
			</div>
		<?php
		}
	}
	...
	/**
	 * Gradient string sanitization
	 *
	 * @param  string	Input to be sanitized
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'skyrocket_gradient_sanitization' ) ) {
		function skyrocket_gradient_sanitization( $input, $setting ) {
			$isValid = true;
			$gradientType = array(
				'linear-gradient',
				'radial-gradient'
			);
			$gradientAngle = skyrocket_get_gradient_angles();
			$gradientParts = skyrocket_get_colors_from_gradient( $input );
			$tmpStr = "";

			// Check gradient starts with either radial-gradient or linear-gradient
			$tmpStr = substr( $input, 0, strpos( $input, '(' ) );
			if ( !in_array( $tmpStr, $gradientType ) ) {
				$isValid = false;
			}
			// Check angle is one of allowed values
			if( $isValid ) {
				if ( !in_array( $gradientParts[0], $gradientAngle ) ) {
					$isValid = false;
				}
			}
			// Check both color stops are valid Hex/RBG/RGBa values
			if( $isValid ) {
				$tmpStr = skyrocket_hex_rgba_sanitization( $gradientParts[1], $setting );
				if ( $tmpStr !== $gradientParts[1] ) {
					$isValid = false;
				}
			}
			if( $isValid ) {
				$tmpStr = skyrocket_hex_rgba_sanitization( $gradientParts[2], $setting );
				if ( $tmpStr !== $gradientParts[2] ) {
					$isValid = false;
				}
			}
	...
	/**
	 * Return an array containing the individual components from a CSS Gradient string
	 *
	 * @param  string	CSS gradient string
	 * @return array	Gradient components
	 */
	if ( ! function_exists( 'skyrocket_get_colors_from_gradient' ) ) {
		function skyrocket_get_colors_from_gradient( $strGradient ) {
			// Tests
			//$strGradient = "linear-gradient(to right bottom, #fdc830, #f37335)";
			//$strGradient = "linear-gradient(to right bottom, rgba(5, 10, 35, 0.5), #f37335)";
			//$strGradient = "linear-gradient(to right bottom, rgb(5, 10, 35), rgba(5, 10, 35, 0.5))";
			//$strGradient = "linear-gradient(to right bottom, #123456, rgba(5, 10, 35, 0.5))";
			//$strGradient = "linear-gradient(to right, #FF0000 0%,rgb(0, 0, 255) 75% )";
			//$strGradient = "linear-gradient(to right, #FF0000 0%, #00FF00 20px, rgb(0, 0, 255) 100%)";
			//$strGradient = "linear-gradient(to right, rgba( 23, 45, 15, .3 ) 0%, #00FF00 20px, rgb(23, 45, 255) 100%)";
			//$strGradient = "linear-gradient(to right, rgba( 23, 45, 15, .3 ) 0%, #00FF00 20px, rgba(40, 90, 150, 0.8) 50%)";
			//$strGradient = "linear-gradient(to right bottom, rgb(5, 10, 35), #fdc830, rgba(5, 10, 35, 0.5))";
			//$strGradient = "linear-gradient(to right bottom, rgb(5, 10, 35), #fdc830, #f37335, rgba(5, 10, 35, 0.5))";
			//$strGradient = "linear-gradient(to right bottom, rgb(5, 10, 35), #fdc830, rgba(5, 10, 35, 0.5), #f37335)";
			//$strGradient = "radial-gradient(circle, rgb(253, 200, 48), rgb(243, 115, 53))";
			...
			$tmpStr = "";
			$tmpStrLen = 0;
			$gradientAngles = skyrocket_get_gradient_angles();

			// Get everything between the braces of a CSS Gradient string
			// e.g. Get 'to right bottom, #123, #456789' from 'background-image:linear-gradient(to right bottom, #123, #456789)' 
			$cssGradient = strstr( $strGradient, "(" );
			$cssGradient = substr( $cssGradient, 1, strrpos( $cssGradient , ")" )-1 );

			// Get gradient angle and convert to nearest string value if degrees were specified
			$pos = strpos( $cssGradient, "," );
			$tmpStr = trim( substr( $cssGradient, 0, $pos ) );
			if ( strpos( $tmpStr, "deg" ) ) {
				$tmpStr = $gradientAngles[intdiv ( intval( $tmpStr ), 45 )];
			}
			$returnVal[] = $tmpStr;
			$cssRemaining = str_split( trim( substr( $cssGradient, $pos+1 ) ) );

			// Get remaining color stops
			$pos = 0;
			foreach( $cssRemaining as $character ) {
				if( $character == "(" ) {
					$skip = true;
				}
				elseif( $character == ")" && $skip )  {
					$skip = false;
				}
				elseif( $character == "," && !$skip ) {
					$commas[] = $pos;
			...
			// Split remaining color stops into individual values
			$pos = 0;
			foreach($commas as $comma) {
				$tmpStr = trim( substr( implode($cssRemaining), $pos, $comma-$pos) );
				// Remove any percentage values from color stops if they exist
				$tmpStrLen = strpos( $tmpStr, ')' );
				if( !$tmpStrLen ) {
					// If it's not an RGB or RGBa value, assume it's Hex and re-check
					$tmpStrLen = strpos( $tmpStr, ' ' );
				} else {
					// If the current color stop is an RGB or RGBa value, add one to length to allow for closing brace
					$tmpStrLen++;
				}
				if( $tmpStrLen > 0 ) {
					$returnVal[] = substr( $tmpStr, 0, $tmpStrLen );
				} else {
					$returnVal[] = $tmpStr;
				}
				$pos = $comma+1;
			}
			// Apply same rules as above to last color stop
			$tmpStr = trim( substr( implode($cssRemaining), $pos ) );
			$tmpStrLen = strpos( $tmpStr, ')' );
			if( !$tmpStrLen ) {
				$tmpStrLen = strpos( $tmpStr, ' ' );
			} else {
	...
	/**
	 * Return an array containing all possible gradient angles
	 *
	 * @return array	List of possible gradient angles
	 */
	if ( ! function_exists( 'skyrocket_get_gradient_angles' ) ) {
		function skyrocket_get_gradient_angles() {
			$gradientAngles = array(
				'to top',
				'to right top',
				'to right',
				'to right bottom',
